  ...
  * `accessories`: A list of `HAP.Accessory` structs to include in this accessory server
  ...
  def compile(%__MODULE__{} = accessory_server) do
    ...
    |> Map.update!(:accessories, fn accessories ->
      accessories
      |> Enum.with_index(1)
      |> Enum.map(fn {accessory, index} -> HAP.Accessory.compile(accessory, index) end)
      |> validate_accessory_ids!()
    end)
  end

  defp validate_accessory_ids!(accessories) do
    aids = Enum.map(accessories, & &1.aid)

    case aids -- Enum.uniq(aids) do
      [] -> accessories
      duplicates -> raise ArgumentError, "duplicate accessory ids #{inspect(Enum.uniq(duplicates))}"
    end
  end
  ...
  def accessories_tree(%__MODULE__{accessories: accessories}, include_values \\ true) do
    %{
      accessories:
        Enum.map(accessories, fn %HAP.Accessory{aid: aid, services: services} ->
          %{
            aid: aid,
            services:
              Enum.map(services, fn %HAP.Service{iid: iid, type: type, characteristics: characteristics} ->
                %{
                  iid: iid,
                  type: type,
                  characteristics:
                    Enum.map(characteristics, fn characteristic ->
                      result = %{
                        iid: HAP.Characteristic.iid(characteristic),
  ...
  def get_characteristics(%__MODULE__{} = accessory_server, characteristics, disposition, opts) do
    ...
    |> Enum.map(fn %{aid: aid, iid: iid} ->
      with {:ok, accessory} <- get_accessory(accessory_server, aid),
           {:ok, characteristic} <- HAP.Accessory.find_characteristic(accessory, iid),
  ...
  def put_characteristics(%__MODULE__{} = accessory_server, characteristics, sender) do
    ...
    |> Enum.map(fn
      %{"aid" => aid, "iid" => iid} = map ->
        with {:ok, accessory} <- get_accessory(accessory_server, aid),
             {:ok, characteristic} <- HAP.Accessory.find_characteristic(accessory, iid) do
          case map do
            %{"value" => value} ->
  ...
  defp get_accessory(%__MODULE__{accessories: accessories}, aid) do
    case Enum.find(accessories, &(&1.aid == aid)) do
      nil -> {:error, -70_409}
