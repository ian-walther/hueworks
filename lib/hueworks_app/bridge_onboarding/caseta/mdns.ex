defmodule Hueworks.BridgeOnboarding.Caseta.Mdns do
  @moduledoc false

  import MdnsLite.DNS

  alias Hueworks.BridgeOnboarding.Caseta.Device

  @service ~c"_lutron._tcp.local"

  def discover do
    query = dns_query(class: :in, type: :ptr, domain: @service)
    response = MdnsLite.query(query, 3_000)
    {:ok, parse(response)}
  end

  def resolve(id, resolver \\ &resolve_hostname/1) when is_binary(id) do
    id = String.downcase(id)

    case resolver.("lutron-#{id}.local") do
      {:ok, ip} ->
        case format_ip(ip) do
          nil -> {:error, :not_found}
          host -> {:ok, %Device{id: id, host: host, name: "Lutron #{id}"}}
        end

      _ ->
        {:error, :not_found}
    end
  end

  def parse(response, resolver \\ &resolve_hostname/1)

  def parse(%{answer: answers, additional: additional}, resolver) do
    records = List.wrap(answers) ++ List.wrap(additional)

    records
    |> ptr_instances()
    |> Enum.map(&device_for_instance(&1, records, resolver))
    |> Enum.reject(&is_nil/1)
  end

  def parse(_response, _resolver), do: []

  defp ptr_instances(records) do
    records
    |> Enum.filter(fn record ->
      dns_rr(record, :type) == :ptr and same_domain?(dns_rr(record, :domain), @service)
    end)
    |> Enum.map(&normalize_domain(dns_rr(&1, :data)))
    |> Enum.uniq()
  end

  defp device_for_instance(instance, records, resolver) do
    srv = find_record(records, instance, :srv)

    case srv && dns_rr(srv, :data) do
      {_priority, _weight, _port, target} ->
        target = normalize_domain(target)

        with id when is_binary(id) <- bridge_id(target),
             host when is_binary(host) <-
               address_for_target(records, target) || resolve_target(resolver, target) do
          %Device{id: id, host: host, name: "Lutron #{id}"}
        else
          _ -> nil
        end

      _ ->
        nil
    end
  end

  defp find_record(records, domain, type) do
    Enum.find(records, fn record ->
      dns_rr(record, :type) == type and same_domain?(dns_rr(record, :domain), domain)
    end)
  end

  defp address_for_target(records, target) do
    case find_record(records, target, :a) do
      nil -> nil
      record -> record |> dns_rr(:data) |> format_ip()
    end
  end

  defp bridge_id(target) do
    case Regex.run(~r/^lutron-([^.]+)(?:\.|$)/i, target, capture: :all_but_first) do
      [id] -> String.downcase(id)
      _ -> nil
    end
  end

  defp format_ip({_, _, _, _} = ip), do: ip |> :inet.ntoa() |> to_string()
  defp format_ip(<<a, b, c, d>>), do: format_ip({a, b, c, d})
  defp format_ip(value) when is_binary(value), do: value
  defp format_ip(_value), do: nil

  defp resolve_target(resolver, target) do
    case resolver.(target) do
      {:ok, ip} -> format_ip(ip)
      _ -> nil
    end
  end

  defp resolve_hostname(hostname) do
    case :inet.getaddr(String.to_charlist(hostname), :inet) do
      {:ok, _ip} = result -> result
      _ -> MdnsLite.gethostbyname(hostname, 3_000)
    end
  end

  defp same_domain?(left, right), do: normalize_domain(left) == normalize_domain(right)

  defp normalize_domain(value) do
    value
    |> to_string()
    |> String.trim_trailing(".")
    |> String.downcase()
  end
end
