...
- Brightness, color, and color temperature are available for entities that support them. Writes are answered immediately and applied off the HomeKit request path. Brightness has been used against Apple Home on hardware; color, temperature, and identity survival across topology changes have not yet been verified there, so treat those as beta.
