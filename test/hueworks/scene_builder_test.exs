defmodule Hueworks.SceneBuilderTest do
  ...
  test "canonical lights and groups are excluded from available lists and validation" do
    ...
    assert state.area_light_ids == [1, 3]
  ...
  test "build reports available lights and groups when nothing assigned" do
    state = Builder.build(area_lights(), groups(), [%{light_ids: []}])
  ...
  test "assigned lights are removed from available lists" do
    ...
    state = Builder.build(area_lights(), groups(), components)
  ...
  test "groups with no area lights do not appear in available groups" do
    state =
      Builder.build(
        area_lights(),
        [
  ...
  test "disabled area lights without an enabled group are excluded from scene requirements" do
    lights = [
  ...
  test "disabled area lights covered by enabled groups remain scene-relevant but not individually selectable" do
    lights = [
      %{id: 1, name: "Lamp", enabled: true},
    ...
    assert state.area_light_ids == [1, 2, 3]
    assert Enum.map(state.available_lights, & &1.id) == [1, 3]
    assert Enum.map(state.available_groups, & &1.id) == [10]
    assert state.unassigned_light_ids == [1, 2, 3]
  end

  test "valid? requires all area lights assigned exactly once" do
    state = Builder.build(area_lights(), groups(), [%{light_ids: [1, 2, 3]}])
    assert state.valid?
  ...
  test "duplicate light assignments invalidate the state" do
    state =
      Builder.build(area_lights(), groups(), [
        %{light_ids: [1, 2]},
        %{light_ids: [2, 3]}
      ])
